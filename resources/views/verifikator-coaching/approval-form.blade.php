...
<div class="p-8">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-8">
        ...
        <div class="p-6">
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="font-mono font-bold text-lg">POD-{{ date('Ymd', strtotime($podcast->tanggal)) }}{{ $podcast->id_podcast }}</div>
                    </div>

                    <!-- Tanggal -->
                    <div class="mb-4">
                        <div class="text-sm text-gray-600 mb-1">Tanggal Booking</div>
                        <div class="font-bold">{{ \Carbon\Carbon::parse($podcast->tanggal)->locale('id')->isoFormat('D MMMM YYYY') }}</div>
                        <div class="text-sm text-gray-600">Hari: {{ \Carbon\Carbon::parse($podcast->tanggal)->locale('id')->isoFormat('dddd') }}</div>
                    </div>
                        ...
                        <div>
                            <span class="px-3 py-1 rounded-full text-sm font-medium 
                                {{ $podcast->status_verifikasi == 'disetujui' ? 'bg-green-100 text-green-800' : 
                                   ...
                                   ($podcast->status_verifikasi == 'penjadwalan ulang' ? 'bg-purple-100 text-purple-800' : 
            ...
            <!-- Approval Form -->
            <form action="{{ route('verifikator-podcast.update-approval', $podcast->id_podcast) }}" method="POST" class="mt-8 border-t border-gray-200 pt-6">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="status_verifikasi" 
                                    id="statusSelect"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500" required>
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="waktu" 
                                ...
                                @foreach($availableTimeSlots as $slot)
                                    <option value="{{ $slot }}" {{ $podcast->waktu == $slot ? 'selected' : '' }}>
                                        {{ $slot }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="text-xs text-gray-500 mt-1">
                                Slot waktu yang tersedia untuk tanggal {{ \Carbon\Carbon::parse($podcast->tanggal)->format('d/m/Y') }}
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="catatan" 
                                      rows="3"
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                      placeholder="Masukkan catatan untuk pemohon">{{ $podcast->catatan }}</textarea>
                        </div>
                    </div>
                </div>
...
document.getElementById('statusSelect').addEventListener('change', function() {
    ...
    if (selectedStatus === 'disetujui') {
        waktuSelect.setAttribute('required', 'required');
        waktuSelect.parentElement.querySelector('label').innerHTML = 'Waktu Podcast *';
    } else {
        waktuSelect.removeAttribute('required');
        waktuSelect.parentElement.querySelector('label').innerHTML = 'Waktu Podcast';
    }
});
